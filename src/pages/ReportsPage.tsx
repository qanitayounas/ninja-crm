        ...
        <div className="flex flex-col gap-8 pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 font-medium">Track where your revenue and real leads come from</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {reportMetrics.map((metric, idx) => (
                    <Card key={idx} className="relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
                        <div className="flex justify-between items-start mb-4">
                            <div className={cn(
                                "h-12 w-12 rounded-22xl flex items-center justify-center", // Note: rounded-22xl as per ninja style or similar
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="font-bold text-ninja-dark text-lg">Distribution by Channel</h3>
                        <span className="text-xs font-bold text-ninja-yellow">34%</span>
                    </div>
                            ...
                            {channelDistribution.map((entry, index) => (
                                <div key={index} className="flex items-center justify-between group cursor-pointer">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest group-hover:text-ninja-dark transition-colors">
                                            {entry.name}
                                        </span>
                                    </div>
                                    <span className="text-xs font-black text-ninja-dark">
                                        {entry.value}%
                                    </span>
                                </div>
                            ))}
            ...
            <Card className="px-0 py-0 overflow-hidden">
                <div className="p-6 border-b border-gray-50 font-bold text-ninja-dark text-lg">
                    Session Events (UTM Tracking)
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-gray-50/50">
                                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Source</th>
                                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Medium</th>
                                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider">Campaign</th>
                                ...
                                <tr key={idx} className="hover:bg-gray-50/30 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm font-medium text-ninja-dark">{event.source}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="text-sm text-gray-400">{event.medium}</span>
                                    </td>
                                    <td className="px-6 py-4 max-w-[150px] truncate">
                                        <span className="text-sm text-gray-400 italic">{event.campaign}</span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="text-sm text-gray-400">{event.content}</span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className="text-sm text-gray-400">{event.term}</span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <Badge status="Active">{event.status}</Badge>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <span className="text-sm font-bold text-green-600">{event.revenue}</span>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
